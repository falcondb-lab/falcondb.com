...
<#
.SYNOPSIS
    FalconDB PoC #5 — Migration: Cleanup (Windows)
.PARAMETER All
...
if ($All) {
    ...
    Remove-Item -Recurse -Force ".\mig_data_falcon" -ErrorAction SilentlyContinue
    ...
    Get-ChildItem $OutputDir -Include "*.log","*.txt","*.json","*.sql" -Recurse | Remove-Item -Force
