#!/usr/bin/env bash
# ============================================================================
# FalconDB DCG PoC — Promote Replica to Primary
# ============================================================================
# After the primary has been killed, this script promotes the replica
# to become the new primary so it can accept writes and queries.
# ============================================================================
set -euo pipefail
...
# Try admin API promote
curl -s -X POST "http://${HOST}:8081/promote" 2>/dev/null || true
sleep 2

# Wait for replica to accept queries
PROMOTE_OK=false
...
echo ""
echo "  New primary available at: psql -h ${HOST} -p ${REPLICA_PORT} -U ${USER} -d ${DB}"
echo ""
